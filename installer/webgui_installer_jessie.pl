...
do {
    # hack
    update( "Skip some tests in testEnvironment.pl" );
    run q{sed -i -e 's/^checkModule("Image::Magick"/# checkModule("Image::Magick"/' /data/WebGUI/sbin/testEnvironment.pl};
    run q{sed -i -e 's/^checkModule("Imager::File::PNG"/# checkModule("Imager::File::PNG"/' /data/WebGUI/sbin/testEnvironment.pl};
